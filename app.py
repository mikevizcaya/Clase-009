...
# ============================================================
st.set_page_config(
    page_title="Markowitz + Black-Litterman v2",
    page_icon="📈",
    layout="wide",
)
...
def sector_weights_table(weights, universe):
    """Agrupa los pesos del portafolio por sector."""
    df = universe[["Ticker", "Sector"]].copy()
    df["Peso"] = np.asarray(weights, dtype=float)

    out = (
        df.groupby("Sector", as_index=False)["Peso"]
        .sum()
        .sort_values("Peso", ascending=False)
        .reset_index(drop=True)
    )
...
# ============================================================
@st.cache_data(ttl=60 * 60 * 6, show_spinner=False)
def get_market_caps(tickers_tuple: tuple[str, ...], last_prices: tuple[tuple[str, float], ...]) -> pd.DataFrame:
    """
    Obtiene capitalización bursátil con varias rutas de Yahoo Finance.

    Orden por activo:
    1) fast_info.market_cap
    2) info.marketCap
    3) sharesOutstanding * último precio disponible
    4) get_shares_full() * último precio disponible

    Devuelve capitalización y método usado por ticker.
    """
    price_map = dict(last_prices)
    rows = []
    ...
    for ticker in tickers_tuple:
        ...
        try:
            ...
            try:
                candidate = tk.fast_info.get("market_cap", np.nan)
            ...
            if not np.isfinite(cap) or cap <= 0:
                try:
                    info = tk.info
                    candidate = float(info.get("marketCap", np.nan))
                    if np.isfinite(candidate) and candidate > 0:
                        cap = candidate
                        method = "Yahoo info"
                except Exception:
            ...
            if not np.isfinite(cap) or cap <= 0:
                try:
                    ...
                    last_price = float(price_map.get(ticker, np.nan))
                    candidate = shares * last_price
                    if np.isfinite(candidate) and candidate > 0:
                        cap = candidate
                        method = "Acciones en circulación × último precio"
                except Exception:
                    pass

            if not np.isfinite(cap) or cap <= 0:
                try:
                    shares_hist = tk.get_shares_full(start=None, end=None)
                    if shares_hist is not None and len(shares_hist.dropna()) > 0:
                        shares = float(shares_hist.dropna().iloc[-1])
                        last_price = float(price_map.get(ticker, np.nan))
                        candidate = shares * last_price
                        if np.isfinite(candidate) and candidate > 0:
                            cap = candidate
                            method = "Histórico de acciones × último precio"
                except Exception:
                    pass
        except Exception:
            pass

        rows.append({"Ticker": ticker, "Capitalización": cap, "Método": method})
        time.sleep(0.03)
...
def build_market_weights(
    ...
    """
    Construye pesos de equilibrio de forma resistente.

    Prioridad global:
    1) Capitalización bursátil Yahoo / reconstruida, si está disponible para TODOS.
    2) Pesos oficiales de los ETFs XLK/XLI guardados en Fase 1.

    Nunca mezcla capitalizaciones con pesos ETF dentro de un mismo cálculo.
    """
    last_prices = []
    for ticker in tickers:
        series = prices[ticker].dropna() if ticker in prices.columns else pd.Series(dtype=float)
        last_prices.append((ticker, float(series.iloc[-1]) if not series.empty else np.nan))

    cap_details = get_market_caps(tuple(tickers), tuple(last_prices))
    caps = cap_details["Capitalización"].reindex(tickers)
    valid_caps = np.isfinite(caps.values) & (caps.values > 0)

    if valid_caps.all():
        weights = caps / caps.sum()
        return weights, "Capitalización bursátil (Yahoo Finance / reconstruida)", cap_details

    if "Peso fondo" in universe.columns:
        etf = universe.set_index("Ticker")["Peso fondo"].reindex(tickers)
        etf = pd.to_numeric(etf, errors="coerce")
        valid_etf = np.isfinite(etf.values) & (etf.values > 0)
        if valid_etf.all() and float(etf.sum()) > 0:
            weights = etf / etf.sum()
            return weights, "Benchmark alternativo: pesos oficiales XLK/XLI", cap_details

    # Compatibilidad con sesiones creadas antes de v2: recuperar Peso fondo
    # desde los rankings guardados, si todavía están disponibles.
    rankings = st.session_state.get("ranking_fase1")
    if isinstance(rankings, dict):
        frames = []
        for df in rankings.values():
            if isinstance(df, pd.DataFrame) and {"Ticker", "Peso fondo"}.issubset(df.columns):
                frames.append(df[["Ticker", "Peso fondo"]])
        if frames:
            fallback = pd.concat(frames, ignore_index=True).drop_duplicates("Ticker")
            etf = fallback.set_index("Ticker")["Peso fondo"].reindex(tickers)
            etf = pd.to_numeric(etf, errors="coerce")
            valid_etf = np.isfinite(etf.values) & (etf.values > 0)
            if valid_etf.all() and float(etf.sum()) > 0:
                weights = etf / etf.sum()
    ...
    missing = caps[~valid_caps].index.tolist()
    raise ValueError(
        "No fue posible construir pesos de mercado. Faltan capitalizaciones para: "
        + ", ".join(missing)
        + " y tampoco hay pesos ETF válidos guardados en la Fase 1."
    )
...
def black_litterman_relative_view(
    ...
    view_variance = (p @ (tau * sigma) @ p.T).item()
...
# ============================================================
    ...
    # --------------------------------------------------------
    # 2.7 DISTRIBUCIÓN DEL PORTAFOLIO POR SECTOR
    # --------------------------------------------------------
    st.divider()
    st.subheader("2.7 Distribución del portafolio por sector")

    sector_sharpe = sector_weights_table(w_sharpe, universe)
    sector_min = sector_weights_table(w_min, universe)

    sector_compare_weights = (
        sector_sharpe.rename(columns={"Peso": "Máximo Sharpe"})
        .merge(
            sector_min.rename(columns={"Peso": "Mínima varianza"}),
            on="Sector",
            how="outer",
        )
        .fillna(0.0)
    ...
    sector_compare_show = sector_compare_weights.copy()
    sector_compare_show["Máximo Sharpe"] *= 100
    sector_compare_show["Mínima varianza"] *= 100
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "Máximo Sharpe": st.column_config.NumberColumn(format="%.2f %%"),
            "Mínima varianza": st.column_config.NumberColumn(format="%.2f %%"),
        },
    )

    sector_graph = sector_compare_show.melt(
        id_vars=["Sector"],
        value_vars=["Máximo Sharpe", "Mínima varianza"],
        var_name="Portafolio",
        value_name="Peso (%)",
    )

    fig_sector_weights = px.bar(
        sector_graph,
        x="Sector",
        y="Peso (%)",
        color="Portafolio",
        barmode="group",
        title="Distribución del capital por sector",
    )
    st.plotly_chart(fig_sector_weights, use_container_width=True)

    # --------------------------------------------------------
    # 2.8 COMPARACIÓN POR SECTOR
    # --------------------------------------------------------
    st.divider()
    st.subheader(
        "2.8 Tecnología vs Industriales vs combinación"
    )
...
# ============================================================
    ...
    with st.spinner("Construyendo pesos de equilibrio..."):
        try:
            market_weights, weight_source, cap_details = build_market_weights(
                tickers=tickers,
                universe=universe,
                prices=prices,
            )
    ...
    if weight_source.startswith("Benchmark alternativo"):
        st.warning(
            "Yahoo Finance no devolvió capitalizaciones válidas para todos los activos. "
            "La Fase 4 continuará usando **pesos oficiales de XLK/XLI** como benchmark "
            "alternativo. No se están mezclando ambos métodos."
        )
    else:
        st.success("Pesos de equilibrio construidos con capitalización bursátil.")

    st.caption(f"**Fuente de pesos de equilibrio:** {weight_source}")
    ...
    market_var = (market_weights.values.T @ cov_matrix.loc[tickers, tickers].values @ market_weights.values).item()
    ...
    market_table["Capitalización"] = cap_details["Capitalización"].reindex(market_table["Ticker"]).values
    market_table["Método capitalización"] = cap_details["Método"].reindex(market_table["Ticker"]).values
    ...
    display_columns = [
        "Ticker", "Nombre", "Sector", "Peso mercado", "π equilibrio (%)"
    ]
    if not weight_source.startswith("Benchmark alternativo"):
        display_columns.insert(3, "Capitalización")

    st.dataframe(
        market_table[display_columns],
        use_container_width=True,
        hide_index=True,
        column_config={
    ...
    with st.expander("Ver diagnóstico de capitalizaciones"):
        diag = cap_details.reset_index().copy()
    ...
    with col_a:
        outperformer = st.selectbox(
            "Activo que creo que tendrá mejor rendimiento",
            options=tickers,
            index=0,
        )
    with col_b:
    ...
    with col_spread:
        spread_pct = st.number_input(
            f"¿En cuántos puntos porcentuales anuales espero que {outperformer} supere a {underperformer}?",
            min_value=0.01,
            max_value=100.0,
            value=5.0,
            step=0.25,
    ...
    with col_conf:
        confidence_pct = st.number_input(
            "¿Qué confianza tengo en esta opinión? (%)",
            min_value=1.0,
            max_value=99.9,
            value=70.0,
            step=1.0,
    ...
    st.info(
        f"**Tu opinión:** esperas que **{outperformer} supere a {underperformer} en "
    ...
    st.subheader("4.5 Distribución por sector — Markowitz vs Black-Litterman")

    sector_markowitz_bl = sector_weights_table(w_markowitz_compare, universe)
    sector_bl = sector_weights_table(w_bl, universe)

    sector_bl_compare = (
        sector_markowitz_bl.rename(columns={"Peso": "Markowitz"})
        .merge(
            sector_bl.rename(columns={"Peso": "Black-Litterman"}),
            on="Sector",
            how="outer",
        )
        .fillna(0.0)
    )
    sector_bl_compare["Cambio (pp)"] = (
        sector_bl_compare["Black-Litterman"]
        - sector_bl_compare["Markowitz"]
    ) * 100

    sector_bl_show = sector_bl_compare.copy()
    sector_bl_show["Markowitz"] *= 100
    sector_bl_show["Black-Litterman"] *= 100
    ...
    sector_bl_graph = sector_bl_show.melt(
        id_vars=["Sector"],
        value_vars=["Markowitz", "Black-Litterman"],
        var_name="Modelo",
        value_name="Peso (%)",
    ...
    fig_sector_bl = px.bar(
        sector_bl_graph,
        x="Sector",
        y="Peso (%)",
        color="Modelo",
        barmode="group",
        title="Distribución sectorial — Markowitz vs Black-Litterman",
    )
    st.plotly_chart(fig_sector_bl, use_container_width=True)

    st.subheader("4.6 Interpretación automática")
